...
public class JwtTokenProvider {
	...
	public String resolveToken(HttpServletRequest request) {
		...
		LOGGER.info("[resolveToken] Request Host : {} ", request.getHeader("Origin"));
		LOGGER.info("[resolveToken] HttpHeaders.AUTHORIZATION : {} ", request.getHeader(HttpHeaders.AUTHORIZATION));
	...
	public Date getExpiration(String token) {
		LOGGER.info("[getExpiration] 토큰 만료시간 조회");
		Jws<Claims> claims = Jwts.parser().setSigningKey(secretKey).parseClaimsJws(token);
		LOGGER.info("[getExpiration] 토큰 만료시간 조회 완료");
		LOGGER.info("[getExpiration] 만료시간 : " + claims.getBody().getExpiration());
